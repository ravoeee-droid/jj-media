(() => {
  if(document.documentElement.dataset.jjBrandRuntime==='20260905-1')return;
  document.documentElement.dataset.jjBrandRuntime='20260905-1';
  const src='/assets/brand/jj-media-logo-20260905.webp?v=1';
  ...
  const apply=()=>{
    document.querySelectorAll('a.logo,a.blog-logo').forEach(link=>{
      ...
      let img=link.querySelector('img.jj-brand-master');
      if(!img){
        img=document.createElement('img');
        img.className='jj-brand-master';
        img.alt='JJ-Media';
        img.width=280;
        img.height=198;
        img.decoding='async';
